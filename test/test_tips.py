...
class TestRelinkedStateMigration:
    """State written before the doc -> doc_link split can keep the curated /
    catalog cross-suppression alive across the upgrade. _load_state migrates
    the two relinked curated tips' state shapes."""
    ...
    def test_collateral_doc_dismissal_lifted(self, tmp_path: Path) -> None:
        """Dismissing the old curated shape recorded the catalog doc; loading
        lifts the doc-level entry while the curated id stays dismissed."""
        st = self._load(tmp_path, {"dismissed": [self._TID], "dismissed_docs": [self._DOC]})
        assert self._TID in st.dismissed
        assert self._DOC not in st.dismissed_docs

    def test_catalog_tips_own_dismissal_preserved(self, tmp_path: Path) -> None:
        """When the catalog's own tip was ALSO dismissed, the user meant it:
        the doc-level entry stays."""
