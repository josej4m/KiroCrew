...
class TestUncProbeGate:
    ...
    def test_unc_shape_detection(self, raw, want):
        assert prompt_blocks._is_unc_shape(raw) is want

    def test_attacker_host_is_refused(self, monkeypatch, tmp_path):
        monkeypatch.setattr(
            "kiro_crew.config.paths.data_home", lambda: tmp_path / "home"
        )
        assert prompt_blocks._unc_probe_allowed(r"\\evil\share\x.png") is False
        assert prompt_blocks._unc_probe_allowed("//evil/share/x.png") is False
    ...
    def test_unc_under_a_unc_data_home_is_allowed(self, monkeypatch):
        ...
        allowed = prompt_blocks._unc_probe_allowed(
            r"\\fileserver\home\me\.kiro\crew\uploads\shot.png"
        )
        forward = prompt_blocks._unc_probe_allowed(
    ...
    def test_sibling_share_on_same_server_is_refused(self, monkeypatch):
        ...
        if os.name == "nt":
            assert prompt_blocks._unc_probe_allowed(r"\\fileserver\other\x.png") is False
