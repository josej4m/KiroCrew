...
class TestLiveScopeNarrowing:
    ...
    def test_source_guard_log_replay_grant_is_audited(self):
        """The subscribe_logs deny branch always called ``_audit_deny``; the
        grant branch that falls through to ``state.subscribe_logs(ws)`` did
        not call anything, leaving every successful subscription with no SEL
        record. Pin the grant call so a future edit cannot drop it silently.
        """
        src = (
            Path(__file__).resolve().parents[1]
            / "src" / "kiro_crew" / "dashboard" / "ws.py"
        ).read_text(encoding="utf-8")
        assert '_audit_allow(ws_app or "<unknown>", "subscribe_logs")' in src, (
            "a granted log subscription must be SEL-audited, matching the deny branch"
        )

    def test_source_guard_periodic_dashboard_grant_is_audited(self):
        """The Tier-0 ``dashboard`` frame is pushed to app sockets every 5s via
        a direct ``ws.send_json`` -- it never reaches ``ws_event_allowed``, so
        without its own call this admission left no SEL record at all.
        """
        src = (
            Path(__file__).resolve().parents[1]
            / "src" / "kiro_crew" / "dashboard" / "ws.py"
        ).read_text(encoding="utf-8")
        assert '_audit_allow(ws_app or "<unknown>", "dashboard")' in src, (
            "the periodic dashboard frame's grant to an app socket must be SEL-audited"
        )
