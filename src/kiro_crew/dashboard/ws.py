        ...
        try:
            while not ws.closed and not shutdown_event.is_set():
                ...
                if not ws.get("_is_dashboard_user", False):
                    ...
                    try:
                        _audit_allow(ws_app or "<unknown>", "dashboard")
                    except Exception:
                        logger.debug(
                            "ws: SEL audit for periodic dashboard grant failed",
                            exc_info=True,
                        )
                try:
                ...
                try:
                    ...
                    if msg_type == "subscribe_logs":
                        ...
                        if not ws.get("_is_dashboard_user", False):
                            ...
                            try:
                                _audit_allow(ws_app or "<unknown>", "subscribe_logs")
                            except Exception:
                                logger.debug(
                                    "ws: SEL audit for subscribe_logs grant failed",
                                    exc_info=True,
                                )
