...
def _unlaunchable_mcp_servers(spec_path: Path) -> str:
    ...
    with no launchable ``command`` validates clean, and the server is then simply
    ...
    Structural only, and narrow on purpose: it reports an entry that cannot start
    under any circumstances (missing/blank/non-string command, or a non-object
    entry). It does not attempt to decide whether a command that IS present will
    resolve on this host, which is a runtime question with a different answer per
    machine and belongs to the binary.

    Fails open on an unreadable or non-JSON file: that is a different fault with
    its own reporting, and guessing here would put a second card on one problem.
    """
    try:
    ...
    for name, entry in sorted(servers.items()):
        ...
        if not isinstance(command, str) or not command.strip():
            return (
                f"The MCP server {name!r} declares no command to run, so Kiro CLI "
                f"cannot start it and the session runs without its tools."
            )
    return ""
