...
import { prepareSendPayload, buildFileLabels, resolveFileSegment, mdImageDest, unwrapMdImageDest } from '../utils/fileTokens'
...
describe('prepareSendPayload', () => {
  ...
  describe('windows image paths (issue #3497)', () => {
    ...
    it('escapes a literal % so the renderer decode is its exact inverse', () => {
      // Without this, a picked file literally named `photo%20copy.png` would
      // decode to `photo copy.png` at render time and fetch the wrong file.
    ...
    it('unwrapMdImageDest is the syntactic inverse of mdImageDest', () => {
      // Percent-free, already-forward-slashed inputs: the % escape belongs to
      // the decode layer (decodeLocalPath) and slash normalization is one-way,
      // so the syntactic wrap/escape layer is what round-trips here.
      for (const p of [
        '/tmp/photo.png',
        ...
        expect(unwrapMdImageDest(mdImageDest(p))).toBe(p)
      }
    })
  })
